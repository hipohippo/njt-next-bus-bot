...
def html_format_path_status_output(status_json: dict) -> str:
    train_status = [
        TrainStatus(
    ...
    html_str = "TO New Jersey:\n" + "\n".join(
        [
            f"{train.line.nj_terminal.display_name}: "
    ...
    html_str += "\n\nTO New York:\n" + "\n".join(
        [
            f"{train.line.ny_terminal.display_name}: "
            f"<b>{int((train.arrival_time - pd.Timestamp.utcnow().tz_convert('America/New_York'))/pd.Timedelta('1 min'))} </b>"
            f"min @ {train.arrival_time.strftime('%H:%M')}"
            for train in to_ny
        ]
